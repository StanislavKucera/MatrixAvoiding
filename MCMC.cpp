...
// Generates random-ish matrix of given size, which is avoiding given general pattern. Uses iter iterations on markov chain.
void MCMCgenerator(size_t iter, general_vector_pair_pattern& pattern, matrix<size_t>& big_matrix)
{
	// random generator from uniform distribution [0, n-1]
	std::random_device rd;
	std::mt19937 rng(rd());
	...
	for (size_t i = 0; i < iter; ++i)
	{
		...
		// test if the changed matrix still avoids the pattern, it obviously holds, when one-entry changes to zero-entry
		if (big_matrix.at(r, c) && !pattern.avoid_from_zero_to_one(r, c, big_matrix))
			// if not return to the previous step
			big_matrix.at(r, c) = big_matrix.at(r, c) ? 0 : 1;
		else if (!big_matrix.at(r, c) && !pattern.avoid_from_one_to_zero(r, c, big_matrix)) {
			assert(!"Something went wrong. After changing one-entry to zero-entry, big_matrix no longer avoids the pattern.");
			throw(!"Something went wrong. After changing one-entry to zero-entry, big_matrix no longer avoids the pattern.");
		}
	}
}
...
// Generates random-ish matrix of given size, which is avoiding given general pattern. Uses iter iterations on markov chain.
void MCMCgenerator(size_t iter, general_map_pattern& pattern, matrix<size_t>& big_matrix)
{
	// random generator from uniform distribution [0, n-1]
	std::random_device rd;
	std::mt19937 rng(rd());
